      ...
      >
        {active?.items.map((item) => {
            ...
            return (
              <li key={item.id}>
                <button
                  ...
                  className="menu-row-premium flex w-full cursor-pointer flex-col gap-2 rounded-[1.2rem] border border-[rgba(255,122,0,0.16)] bg-[rgba(0,0,0,0.3)] px-5 py-4 text-left backdrop-blur-sm transition-[transform,border-color] hover:-translate-y-0.5 hover:border-[rgba(255,122,0,0.45)] sm:flex-row sm:items-start sm:justify-between sm:gap-6"
                ...
                >
                  ...
                  <div className="flex shrink-0 flex-col items-end gap-1 sm:pt-0.5">
                    <p className="text-right text-lg font-bold text-[var(--cj-orange)]">{item.price}</p>
          ...
          return (
            <li
              key={item.id}
              className="menu-row-premium flex flex-col gap-2 rounded-[1.2rem] border border-[rgba(255,122,0,0.16)] bg-[rgba(0,0,0,0.3)] px-5 py-4 backdrop-blur-sm sm:flex-row sm:items-start sm:justify-between sm:gap-6"
            >
              <div>
                <h3 className="text-[var(--cj-cream)]">{item.name}</h3>
                {item.description ? <p className="mt-1 max-w-xl text-sm leading-relaxed text-[var(--cj-cream)]/70">{item.description}</p> : null}
